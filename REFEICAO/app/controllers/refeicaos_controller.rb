class RefeicaosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @refeicao.save
        ...
        format.json { render action: 'show', status: :created, location: @refeicao }
      else
  ...
  def destroy
    @refeicao.destroy
    respond_to do |format|
      format.html { redirect_to refeicaos_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def refeicao_params
      params.require(:refeicao).permit(:DATE_Refeicao, :DESC_Refeicao, :TIPO_Refeicao, :DESC_Acompanhamento, :VALOR_Refeicao, :VALOR_Acompanhamento, :DESC_Observacao)
    end
end
